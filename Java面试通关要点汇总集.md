...
- **Map的功能**
  ...
  ![img](https://user-gold-cdn.xitu.io/2018/4/9/162a98c4bf76e0d9?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

- **散列表介绍**

  首先回顾一下数据和链表：

  - 链表和数组都可以按照人们的意愿来排序元素的次序，他们可以说是有序的（存储的顺序和取出的顺序是一致的）。
  - 但同时，带来缺点：想要获取某个元素，就要访问所有的元素，直到找到为止。
  - 这会让我们消耗很多的时间在里边，遍历访问元素。

  而还有另外一些存储结构：不在意元素的顺序，能够快速地查找元素的数据
...
- **散列表工作原理**
  ...
  ![img](https://user-gold-cdn.xitu.io/2018/4/9/162a98c4bfcecf91?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

  一个桶上可能会遇到被占用的情况（hashCode散列码相同，就存储在同一个位置上），这种情况是无法避免的，这种现象称之为：散列冲突。

  - 此时需要用该对象与桶上的对象进行比较，看看该对象是否存在桶上，如果存在，就不添加，如果不存在则添加到桶上。
  - 在JDK1.8中，桶满了会从链表变成平衡二叉树。
...
- **红黑树介绍**
  ...
  ![img](https://user-gold-cdn.xitu.io/2018/4/9/162a98c4c013be70?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

  - 回顾二叉查找树

    利用二叉查找树的特性，一般来说可以很快地查找出对应地元素。

    可是二叉查找树也有最坏的情况：

    ![img](https://user-gold-cdn.xitu.io/2018/4/9/162a98c4bff5dc04?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

    上面符合二叉树的特性，但是它是线性的，完全没有树的用处。

    因此有了平衡树的概念。红黑树就是一种平衡树。

  - 知新2-3树

    讲到了平衡树就不得不说最基础的2-3树：

    ![img](https://user-gold-cdn.xitu.io/2018/4/9/162a98c5343df147?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)
    ...
    - 合并2-节点为3-节点，将3-节点扩充为4-节点。
    - 分解4-节点为3-节点，3-节点为2-节点。
    - 至使得树平衡。

  - 从2-3树到红黑树

    由于2-3树为了保持平衡性，在维护的时候需要大量的节点交换，这些变换在实际代码中是很复杂的，所以在2-3树的理论基础上发明了红黑树。

    - 红黑树是对2-3查找树的改进，它能用一种统一的方式完成所有变换。

    ![img](https://user-gold-cdn.xitu.io/2018/4/9/162a98c527fa9621?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

    我们将红色节点的左连接画平看看：

    ![img](https://user-gold-cdn.xitu.io/2018/4/9/162a98c5345d5cfb?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

    一颗典型的二叉树：

    ![img](https://user-gold-cdn.xitu.io/2018/4/9/162a98c544a2fa28?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

    将红色节点的左连接画平之后，得到2-3树：

    ![img](https://user-gold-cdn.xitu.io/2018/4/9/162a98c5802d3394?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

  - 红黑树基础知识

    红黑树用的是两种方式来替代2-3树不断的节点交换操作：

    - 旋转：顺时针旋转和逆时针旋转
    - 反色：交换红黑的颜色

    红黑树为了保持平衡，还有制定一些约束，遵守这些约束才能叫做红黑树：
...
- **底层数据结构分析**
  ...
  - HashMap源码分析
    ...
    - 如果定位到的数组位置有元素就和要插入的key比较，如果key相同就直接覆盖，如果key不相同，就判断p是否是一个树节点，如果是就调用e = ((TreeNode<K,V>)p).putTreeVal(this, tab, hash, key, value)将元素添加进入。如果不是就遍历链表插入（插入的是链表尾部）。
      ...
      再来对比一下JDK1.7 put方法的代码
  
      对于put方法的分析如下：
  
      - 如果定位到数组位置没有元素。就直接插入。
      - 如果定位到数组位置有元素，遍历以这个元素为头节点的链表，依次和插入的key比较，如果key相同就直接覆盖，不同就采用头插法插入元素。
      ...
      推荐阅读：[吊打系列-HashMap](https://juejin.im/post/5dee6f54f265da33ba5a79c8)

#### ConcurrentHashMap

- **为什么选择ConcurrentHashMap**

  HashMap在对线程环境下存在线程安全问题，一般都是怎么处理这种情况呢？

  - 使用Collections.synchronizedMap(Map);创建线程安全的Map集合
  - HashTable
  - ConcurrentHashMap

  处于线程并发度的原因，都会舍弃前两者使用最后的。

  推荐阅读：[吊打系列-ConcurrentHashMap](https://juejin.im/post/5df8d7346fb9a015ff64eaf9)